...
//    ground = world.createBody(bd);
  ...
  void loadLevel([int level]) {
    ...
    this.world = new World.withPool(gravity,
...
//    this._createGround(groundHeight);
    ...
    this.lightEngine = new LightEngine(querySelector("#shadow_canvas"),
        this.ctx, this.groundLevel + groundHeight);

    this.level = Levels.getLevel(level);

    this.sun = new Vector2(
        this.level['sun_x'], Game.canvasCenter.y / Game.VIEWPORT_SCALE);
...
//    List points = level['start'];
    ...
    this.endSolver = new EndSolver(endVectorPoints, new Vector2(
        this.level['critters']['spawn_point'][0],
        this.level['critters']['spawn_point'][1]), this.critters, this);
...
//      print(e.keyCode);
    ...
    window.onKeyUp.listen((e) {
      if (this.eventHandler.dragHandler.isActive()) {
        ...
        this.eventHandler.dragHandler
            .getActiveObject().body.angularVelocity = 0.0;
      }
    });
...
//    });
  ...
  void _createGround(double height, List moreGrounds) {
    StaticBox ground;
    ground = new StaticBox(new Vector2(50.0, height / 2),
        new Vector2(0.0, this.groundLevel + height / 2));
    ground.addObjectToWorld(this.world);
    this.grounds.add(ground);

    ground = new StaticBox(new Vector2(50.0, 1.0),
        new Vector2(0.0, Game.canvasCenter.y / Game.VIEWPORT_SCALE));
    ground.addObjectToWorld(this.world);
    this.grounds.add(ground);

    ground = new StaticBox(
        new Vector2(1.0, (Game.canvasCenter.y * 2) / Game.VIEWPORT_SCALE),
        new Vector2(Game.canvasCenter.x / Game.VIEWPORT_SCALE, 0.0));
    ground.addObjectToWorld(this.world);
    this.grounds.add(ground);
    ...
    for (List groundDefinition in moreGrounds) {
      ground = new StaticBox(
          new Vector2(groundDefinition[2], groundDefinition[3]),
...
//      double test = boxDefinition[4];
      ...
      box = new DynamicBox(new Vector2(boxDefinition[2], boxDefinition[3]),
          new Vector2(boxDefinition[0], boxDefinition[1]), boxDefinition[5],
          boxDefinition[6], boxDefinition[4]);
...
//    print("${Game.canvasCenter.toString()} ${Game.canvasOffset.toString()}");
  ...
  void startCritters() {
    ...
    Vector2 spawnPoint = new Vector2(
        crittersSettings['spawn_point'][0], crittersSettings['spawn_point'][1]);
    ...
    for (int i = 0; i < crittersSettings['count']; i++) {
      ...
      randomPosition.x = randomPosition.x +
          this.randomGenerator.nextDouble() * CRITTER_SPAWN_RADIUS;
...
//      double dist = this.eventHandler.dragHandler.objectDistanceToDestination();
      ...
      Vector2 diffVector = new Vector2.copy(
          this.eventHandler.dragHandler.getCorrectedDestination());
      diffVector.sub(obj.body.position);

      obj.body.linearVelocity = new Vector2(
          diffVector.x * Game.MOVE_VELOCITY, diffVector.y * Game.MOVE_VELOCITY);
    ...
    window.requestAnimationFrame((num time) {
      ...
      if (Debug.isEnabled()) {
        this.debug.endSolverElapsedUs =
            this.debug.endSolverStopwatch.elapsedMicroseconds;
      }
...
//    ctx.clearRect(0, 0, this.canvas.width, this.canvas.height);
    ...
    if (Debug.showTextures) {
      ...
      if (Debug.isEnabled()) {
        this.debug.staticDrawElapsedUs =
            this.debug.staticDrawStopwatch.elapsedMicroseconds;
      }
    } else {
      world.drawDebugData();
    }
...
//    if (Debug.showShadows) {
    if (Debug.isEnabled()) {
      this.debug.dynamicDrawElapsedUs =
          this.debug.dynamicDrawStopwatch.elapsedMicroseconds;
...
//    }
  ...
  void updateProgressElm() {
    ...
    this.progressElm.text =
        "${crittersPassed}/${this.level['critters']['count']}";
  }
...
//  }
  ...
  static Vector2 convertCanvasToWorld(Vector2 canvasVectorOrPoint) {
    return new Vector2(
        (canvasVectorOrPoint.x - Game.canvasCenter.x) / Game.VIEWPORT_SCALE,
        (Game.canvasCenter.y - canvasVectorOrPoint.y) / Game.VIEWPORT_SCALE);
  }

  static void convertWorldToCanvas(Vector2 worldVectorOrPoint) {
    worldVectorOrPoint.x =
        worldVectorOrPoint.x * Game.VIEWPORT_SCALE + Game.canvasCenter.x;
    worldVectorOrPoint.y =
        -worldVectorOrPoint.y * Game.VIEWPORT_SCALE + Game.canvasCenter.y;
  }
